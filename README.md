...
This repository demonstrates a way to deploy Shinylive applications to GitHub Pages.
...
At this point, you have two options for deploying the application.

1. Commit the `docs/` directory and push to GitHub.
2. Don't commit `docs/`, and instead use GitHub Actions to run `shinylive export` and deploy to GitHub Pages every time you push new commits to the repository.

Both options require some configuration of the repository's GitHub Pages settings.
...
The main advantage of (2) is that the repository can remain very small. (This example repository contains only a few small text files.) It will also be rebuilt every time you push commits to GitHub, so you can be sure that the resulting web site is up-to-date with minimal effort. The drawback is that the `docs/` directory is not committed to the repository, so you cannot easily see the exact files that are deployed, nor can you retrieve old versions of them from the repository.

This example repository does option (2).
...
You can [publish to GitHub Pages](https://docs.github.com/en/pages/getting-started-with-github-pages/configuring-a-publishing-source-for-your-github-pages-site#publishing-with-a-custom-github-actions-workflow) using GitHub Actions. (Note that this feature is currently in beta as of 2022-09).
